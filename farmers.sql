...
CREATE TABLE `user` (
  `id` int(11) NOT NULL,
  `username` varchar(50) NOT NULL,
  -- 🔑 CHANGE: Added phone column as UNIQUE and NOT NULL
  `phone` varchar(15) NOT NULL UNIQUE, 
  -- 🔑 NOTE: Email field kept, but UNIQUE constraint is now on phone
...
--
-- NOTE: If you use this script, you will need to manually update existing 'user' records 
-- (like the one below) to add a 'phone' value before re-dumping the data.
-- Since the phone number is required, I've left the INSERT data out here.

-- --------------------------------------------------------
-- 
-- Indexes and AUTO_INCREMENTs for dumped tables (UNCHANGED)
-- 
-- --------------------------------------------------------
...
ALTER TABLE `user`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `phone` (`phone`); -- Added index for the new unique phone column
