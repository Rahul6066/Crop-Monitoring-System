...
class User(UserMixin,db.Model):
    id=db.Column(db.Integer,primary_key=True)
    username=db.Column(db.String(50))
    # phone is UNIQUE but now nullable=True for flexible signup
...
class Field(db.Model):
    ...
    # Link field to the currently logged-in user
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    name = db.Column(db.String(100), nullable=False)
    size = db.Column(db.Numeric(10, 2), nullable=False)
    unit = db.Column(db.String(10), default='Acre') # e.g., Acre, Hectare
    crop_planted = db.Column(db.String(100), nullable=True)
...
def add_field():
    if request.method == 'POST':
        name = request.form.get('name') # Changed from field_name to name to match template
        size = request.form.get('size')
        unit = request.form.get('unit')
        crop_planted = request.form.get('crop_planted')
        
        if not name or not size:
            flash("Field Name and Size are required.", "danger")
        ...
        try:
            new_field = Field(
                ...
                size=size,
                unit=unit,
                crop_planted=crop_planted if crop_planted else None
            )
...
# ... (rest of the older routes remain unchanged)
...
if __name__ == '__main__':
    with app.app_context():
        # db.create_all() MUST run to create the new 'Field' table and update User constraints
        db.create_all() 
